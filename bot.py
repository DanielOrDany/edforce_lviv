...
API_TOKEN = '886297914:AAH6tCKQIFowXQSWd5AqCp8iJmyMx3d9Ow4'
...
def create_question_markups():
    ...
    for i in range(len(db_elements)):
        ...
        for x in range(len(answers)):
            answer = types.InlineKeyboardButton(text=f'{answers[x]}', callback_data=f'{x}')
...
def send_welcome(message):
    congrat_message = f'Hello {message.from_user.first_name}!\nChoose what you want to do in a menu.'
    ...
    if not is_testing:
            ...
            if is_inserting_data:
                ...
                try:
                    con = psycopg2.connect(database="d99gqkbd6vao8t",
                                           ...
                                           port="5432")
                    cursor = con.cursor()
                    cursor.execute(f"INSERT INTO user_test_results (id, contact_data, test_code, results) VALUES ('{str(uuid.uuid4())}', '{message.text}', '{user_test_code}', '{user_right_answers}/{len(right_answers)}')")
                    con.commit()
                    cursor.close()
        ...
        try:
            con = psycopg2.connect(database="d99gqkbd6vao8t",
                                   ...
                                   port="5432")
            cursor = con.cursor()
            cursor.execute(f"SELECT * FROM tests WHERE test_code='{message.text}' AND status='active'")
...
def callback_handler(call):
    ...
    if call.data == 'end_test':
        ...
        for i in range(len(results)):
            result_string += f'Question {i + 1}:\nYour answer: {user_answers[i]} | Right answer: {right_answers[i]}\n\n'
        for i in results:
            if i == 'Right':
                num_right_answers += 1
        global user_right_answers
        user_right_answers = num_right_answers
        result_string += f'Your result: {num_right_answers}/{len(right_answers)}'
    ...
    try:
        if isinstance(int(call.data), int):
            ...
            for x in range(len(answers)):
                if x == int(call.data):
                    answer = types.InlineKeyboardButton(text='{' + answers[x] + '}', callback_data=f'{x}')
